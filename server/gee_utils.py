...
def get_flash_flood_risk(bbox: List[float]) -> Dict[str, Any]:
    ...
    is_arid_desert = (
        ...
        (30.0 <= center_lat <= 42.0 and -118.0 <= center_lon <= -102.0) or  # US Southwest Desert
        (35.0 <= center_lat <= 48.0 and 55.0 <= center_lon <= 105.0) or  # Central Asian Steppe / Gobi
        (-30.0 <= center_lat <= -15.0 and -75.0 <= center_lon <= -65.0)  # Atacama
    )
    ...
    if is_himalayan:
        ...
        deforestation_pct = rng.uniform(32.0, 58.0)  # Heavy slope deforestation amplifies landslide & flood
    ...
    if _FLOOD_MODEL and "weights" in _FLOOD_MODEL:
        ...
        pred_prob = intercept + (w.get("Deforestation", 0.0056) * defor_feat) + \
                    ...
                    (w.get("Urbanization", 0.0056) * urban_feat) + \
